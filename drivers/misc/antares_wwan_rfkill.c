...
static int antares_rfkill_probe(struct platform_device *pdev)
{
    ...
    dev->wwan_rfkill_state = 1;
    dev->wwan_rfkill_state = 1;
	rfkill_init_sw_state(dev->wwan_rfkill, false);			// RF is not blocked
    rfkill_init_sw_state(dev->gps_rfkill, false);

    gpio_direction_output(dev->disable_pin, 1); /* power on module */
