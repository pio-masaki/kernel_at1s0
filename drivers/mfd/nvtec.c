...
/* Daniel Wang add for scorpio kernel wakelock >>> */
...
static int NVTEC_MAX_FW_SIZE = 65536;
/* Daniel Wang add for Scorpio <<< */

/* Daniel Wang add for scorpio APK and kernel wakelock >>> */
unsigned int percentage = 0;
static struct wake_lock upgrade_lock;
static NvBool upgrade_lock_flag = NV_FALSE;
/* Daniel Wang add for scorpio APK and kernel wakelock <<< */

/* Daniel Wang add for scorpio mcu led mode control >>> */
struct mcu_led_info{
    uint16_t raw;
    char *name;
};
...
static int nvtec_get_firmware_version(struct i2c_client *client, uint8_t *ver_str)
{
	...
	/* Daniel Wang add for Scorpio >>> */
	#if 1
	if ((version[15]=='5') && (version[16] =='8'))
	{
		printk("nvtec: M058LAN 32KB \n");
		NVTEC_MAX_FW_SIZE = 32768;
	}
	else if ((version[15]=='5') && (version[16] =='A'))
	{
		printk("nvtec: M0516LAN 64KB \n");
		NVTEC_MAX_FW_SIZE = 65536;
	}
	else
	{
		NVTEC_MAX_FW_SIZE = 65536;
...
static int nvtec_enter_flash_mode(struct i2c_client *client, uint16_t magic)
{
    ...
    /* Daniel Wang add for scorpio APK >>> */
    percentage = 0;   
    /* Daniel Wang add for scorpio APK <<< */
...
static int nvtec_set_state(struct i2c_client *client, uint16_t state)
{
    ...
    mutex_lock(&nvtec->lock);
    if (nvtec->state == NVTEC_STATE_S3 && state == NVTEC_STATE_S0) { /* S3->S0 */
        gpio_set_value(nvtec->request_pin, 1);
        msleep(1);
        gpio_set_value(nvtec->request_pin, 0);
        msleep(5);
    } else if (nvtec->state == NVTEC_STATE_S4 && state == NVTEC_STATE_S0) { /* S4->S0 */
        gpio_set_value(nvtec->request_pin, 0);
        msleep(1);
        gpio_set_value(nvtec->request_pin, 1);
        msleep(5);
	...
	if (state == NVTEC_STATE_S0){
		...
		gpio_set_value(nvtec->request_pin_1, 1);
		gpio_set_value(nvtec->request_pin_2, 1);
		//ret = __nvtec_write_word(client, 0xd1, state);
                msleep(30);
...
{
    return __nvtec_read_word(client, 0xd2, mode);
}
...
static int nvtec_set_hdmi_sw_mode(struct i2c_client *client, uint16_t mode)
{
    ...
    return __nvtec_write_word(client, 0xd2, mode);
}
...
static int nvtec_set_hdmi_p1_strength(struct i2c_client *client, int strength)
{
    ...
    return __nvtec_write_word(client, 0xd3, value);
}
...
/* Daniel Wang add for scorpio led control >>> */
...
static int nvtec_transfer_firmware(struct i2c_client *client, uint8_t *buffer, int len)
{
    ...
    while (byte_transferred != len) {
		...
		percentage=((byte_transferred*100)/NVTEC_MAX_FW_SIZE);	/* Daniel Wang add for scorpio */
        
        printk("nvtec:(%d) byte_transferred= %d \n",percentage ,byte_transferred);
        wait_event(nvtec->wq_ack, cmd_ack == 1);
...
/* Daniel Wang add for scorpio led control and upgrade state >>> */
/*
static ssize_t nvtec_led_control_show(struct device *dev,
                                struct device_attribute *attr,
                                char *buf)
{
    int ret,i;
    ...
    ret = nvtec_get_led_mode(nvtec->client,&mode);
    if (ret < 0)
        return 0;
    ...
    //printk("[0x%x]\n",mode);
    for (i=0;i<5;i++){
    	if (mode==led_mode[i].raw)
			return sprintf(buf,"mode=%s\n",led_mode[i].name);
    }
    return sprintf(buf, "mode=unknow [0x%x]\n",mode);
    #endif
}

static ssize_t nvtec_led_control_store(struct device *dev,
                                struct device_attribute *attr,
                                char *buf,
				size_t count)
{
    ...
    next = (char *)buf;
    cp = strsep(&next, delim);
    if (NULL == cp || NULL == next)
        return count;
	...
	#if 0
    if (!strncmp(cp, "mode", 4)) {
	if (!strncmp(next, "on", 2)){
		config=0x150;
		ret = nvtec_set_led_mode(nvtec->client,config);
		if (ret <0)
		  return count;
		printk(" ! led on mode ! \n");
                ...
                if (ret <0)
                  return count;
		printk(" ! led off mode ! \n");
	}else if (!strncmp(next, "blink_1", 7)){
                config=0x155;
                ret = nvtec_set_led_mode(nvtec->client,config);
                if (ret <0)
                  return count;
		printk(" ! led blinking 1 mode ! \n");
	}else if (!strncmp(next, "blink_2", 7)){
                config=0x156;
                ret = nvtec_set_led_mode(nvtec->client,config);
                if (ret <0)
                  return count;
		printk(" ! led blinking 2  mode ! \n");
	}else if (!strncmp(next, "mcu", 3)){
		config=0;
                ret = nvtec_set_led_mode(nvtec->client,config);
                if (ret <0)
                  return count;
		printk(" ! led control by mcu ! \n");
	}else{
		printk(" ! unknow mode ! \n");
	}
    }else{
	printk(" ! unsupport mode ! \n");
    }
    return count;
	#else
    for (i=0;i<5;i++){
    	//printk(" %d  %d raw data [%X] = %s \n",(unsigned long)strlen(led_mode[i].name),sizeof(led_mode),led_mode[i].raw,led_mode[i].name);
    	if (!strncmp(next,led_mode[i].name,(unsigned long)strlen(led_mode[i].name))){
			ret = nvtec_set_led_mode(nvtec->client,led_mode[i].raw);
			return count;
		}
    }
    printk("mode=unknow\n");
...
/* Daniel Wang add for scorpio led control and upgrade state >>> */
//static DEVICE_ATTR(led_ctl, S_IWUSR | S_IRUGO, nvtec_led_control_show, nvtec_led_control_store);
...
static int nvtec_add_sysfs_entry(void)
{
    ...
    /* Daniel Wang add for scorpio led control and upgrade state >>> */
    /*
    ret = device_create_file(nvtec_dev.this_device, &dev_attr_led_ctl);
    if (ret < 0)
        return ret;
    */
...
static void nvtec_remove_sysfs_entry(void)
{
    ...
    /* Daniel Wang add for scorpio led control and upgrade state >>> */
    //device_remove_file(nvtec_dev.this_device, &dev_attr_led_ctl);
    device_remove_file(nvtec_dev.this_device, &dev_attr_up_state);
    /* Daniel Wang add for scorpio led control and upgrade state <<< */
}
                                     ...
                                     const struct i2c_device_id *id)
{
    ...
    /* Daniel Wang add for scorpio >>> */
    ec->request_pin_1 = pdata->request_pin_1;
    ec->request_pin_2 = pdata->request_pin_2;
    /* Daniel Wang add for scorpio <<< */

    gpio_set_value(ec->request_pin_1, 1);
    gpio_set_value(ec->request_pin_2, 1);
    ...
    /*
    * Scorpio Need to remove Daniel Wang 
    */
    ...
    /* [Patch from Ants] Ignore access fail of the EC firmware version and continue loading driver */
    ...
    /* Scorpio configure default suspened led behavior Daniel Wang >>> */
...
static int __devexit nvtec_i2c_remove(struct i2c_client *client)
{
    ...
    /* Daniel Wang add for scorpio kernel wakelock >>> */
    wake_lock_destroy(&upgrade_lock);
    /* Daniel Wang add for scorpio kernel wakelock <<< */ 


    gpio_free(ec->request_pin_1);
    gpio_free(ec->request_pin_2);
...
{
    /* [Patch from Ants] Disable EC AP_WAKE# interrupt during system shutdown */
    if (client->irq)
