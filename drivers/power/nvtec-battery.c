...
#include <linux/timer.h>/*Jolen peng add 11.0602*/
...
static int ac_in_source = 0; /*Jolen peng add 11.0601*/
static int hw_ver_mcu = 0; /*Jolen peng add 11.0601, get from mcu system event*/
...
static int usb_power_get_property(struct power_supply *psy, /*Jolen peng modify 11.0601*/
	enum power_supply_property psp, union power_supply_propval *val);
...
static struct power_supply nvtec_supplies[] = {
    ...
    {
        .name = "usb",			 /*Jolen peng modify 11.0601*/
...
{
    return gpio_get_value(batt_dev->ac_in_pin);		/* Daniel Wang add for scorpio */
}
...
static int nvtec_battery_get_capacity(uint16_t *value)
{
 ...
 retry:
    ...
    if ((retry != 0) && ((cap > 100) || ((cap < 5) && (!is_ac_online())))) {
        retry --;
...
{
    return nvtec_read_word(batt_dev->nvtec_dev,
                           SBS_AVG_CURRENT,	/* Daniel Wang add for scorpio */
                           value);
}
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_STATUS:
        ...
        if (is_ac_online())
            if (value & SBS_STATUS_DISCHARGING)  /*Jolen peng modify 11.0601*/
...
static int nvtec_battery_thread(void *args)
{
    ...
    do {
		...
		power_supply_changed(&nvtec_supplies[2]);

        is_psy_changed = 0;
#if defined(CONFIG_TOUCHSCREEN_PM)		
	nvtec_battery_get_capacity(&value);
	ac_online=is_ac_online();
	mxt_power_manager(ac_online, value);
#endif		
...
static int nvtec_battery_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&nvtecb_lock, WAKE_LOCK_SUSPEND, "NvBattLowSuspendLock");
...
static int nvtec_battery_remove(struct platform_device *pdev)
{
    ...
    kfree(batt_dev);
	del_timer(&mcu_sys_event_delay_timer);
    return 0;
}
