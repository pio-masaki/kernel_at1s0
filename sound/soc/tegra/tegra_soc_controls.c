...
ssize_t tegra_sound_path_show(struct device *dev, struct device_attribute *attr, char *buf)
{
       int EXT_HEADSET = 0;
	int INT_HEADSET = 0;

	EXT_HEADSET = gpio_get_value(88);
	INT_HEADSET = !gpio_get_value(178);

       if(INT_HEADSET)
               sprintf(buf, "MB Headphone\n");
       else if(EXT_HEADSET)
               sprintf(buf, "Dock Headphone\n");
       else
               sprintf(buf, "MB Speaker\n");

       return strlen(buf);
}
DEVICE_ATTR(output_path, S_IRUGO, tegra_sound_path_show, NULL);
...
int tegra_controls_init(struct snd_soc_codec *codec)
{
	...
	err = device_create_file(codec->dev, &dev_attr_output_path);
       if (err) {
             pr_err("%s: add_sysfs_entry info failed\n", __FUNCTION__);
	}
